...
def list_display(
    title: str, items: List[str], prefix: str = ""
) -> str:
    items = items or ["None"]
    return f"{title}:\n" + "\n".join(
        [f"  {prefix}{i}" for i in items]
    )


def fmt_uses(uses):
    lines = [
        f"    {argv[0]} <config_path> [options] " + l for l in uses
    ]
    return "Usage:\n" + "\n".join(lines)
...
def parse_cli_params(cli_params) -> Tuple[str, dict, dict]:
    ...
    options = {
        k[2:]: v for k, v in cli_params.items() if has_prefix(k, "--")
    }
...
def resolve_job_matches(
    ...
    chosen_routine = next(
        (routine for routine in routines if routine.name == choice),
        None,
    )
    chosen_jobs = [
        job
        for job in jobs
        if chosen_routine
        and job.name in chosen_routine.jobs
        or job.name == choice
    ]
    return chosen_jobs
